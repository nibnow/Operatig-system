...
{ 
  ...
  //todo parsing file_name
  char temp[256];
  char *ret, *next;
  char argv[256];

  strlcpy(temp, file_name, strlen(file_name) + 1);
  ret = strtok_r(temp, " ", &next);
  strlcpy(argv, ret, strlen(ret) + 1);
...
load (const char *file_name, void (**eip) (void), void **esp) 
{
  ...
  //To do : parse file name
  //printf("\nparisng!!!\n");

  int argc = 0;
  char **argv;
  char temp[256];//to save original string
  char *ret, *next;
 ...
 // strcat(temp, " \0");
  ...
  for(int i = argc - 1; i >= 0; i--){
    ...
    *esp -= cur_len;
    total_len += cur_len;
    strlcpy(*esp, argv[i], cur_len);
    argv[i] = *esp;
  }

  if(total_len % 4 != 0) {//word alignment
    *esp -= 4 - (total_len % 4);
    //**(uint32_t **)esp = 0;
  }
